            ...
            .then((favorite) => {
                ...
                if (favorite) {
                    req.body.forEach((fav) => {
                        //if favorite not present then add the new fav
...
favoriteRouter.route('/:campsiteId')
    ...
    .get(cors.cors, authenticate.verifyUser, (req, res, next) => {
        res.statusCode = 403;
        res.end(`GET operation not supported on /favorites ID ${req.params.campsiteId}!`);
    })
    .post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
        //check for users favs list
        Favorite.findOne({ user: req.user._id })
            .then((favorite) => {
                if (favorite) {
                    // favorite not present then push the new fav in
    ...
    .put(cors.corsWithOptions, authenticate.verifyUser, (res, req, next) => {
        res.statusCode = 403;
        res.end('PUT operations do not work on /favorites');
    })
    .delete(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
        Favorite.findOne({ user: req.user._id })
            .then((favorite) => {
